...
1. **The board's real reply token.** The expected `SY@` / `AI@` / `AL@` were read out of the Python
   ...
   either `AI@` or `AI;5=O@` and logs a warning naming the token it actually saw. Anything that is
   not an acknowledgement reaches the log here verbatim. **Read it.** If the board answers in a third
...
"@eai/serial":     "../hardware-libs/serial/mod.ts",
"@eai/shared":     "../hardware-libs/shared/mod.ts",
"@eai/logging-ts": "jsr:@eai/logging-ts@^2.6.0",
"@eai/async":      "jsr:@eai/async@^1.0.0",
"@eai/hotplug":    "jsr:@eai/hotplug@^1.0.0",
"@std/bytes":      "jsr:@std/bytes@^1.0.6",
```

Six extra entries, not one: a relative path brings the driver's source but not the workspace import
...
| `src/rows.ts`          | Payload → button grid, and observations → the markdown record.    |
